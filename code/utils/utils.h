...
inline float generateRandomNumber(float a, float b, int n_decimales, std::mt19937& gen) {
	std::uniform_real_distribution<float> dis(a, b);
	float num = dis(gen);

	// Round only if n_decimales >= 0
	if (n_decimales >= 0) {
		float factor = std::pow(10.0f, n_decimales);
...
inline int generateRandomInt(int a, int b, std::mt19937& gen) {
