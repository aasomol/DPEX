...
void GroundedNumericExpression::copyFrom(GroundedNumericExpression* c)
{
    type = c->type;
    value = c->value;
    index = c->index;
    terms.clear();
    for (GroundedNumericExpression& t : c->terms) {
        terms.emplace_back();
...
void GroundedNumericCondition::copyFrom(GroundedNumericCondition* c)
{
...
void GroundedNumericCondition::reshape(GroundedVar* v)
{
	if (terms[1].requiresNumericVariable(v->index))
        swapTerms();
    reshapeTerms(v);
}

void GroundedNumericCondition::swapTerms()
{
    terms.emplace_back();
...
void GroundedNumericCondition::swapCondition()
{
    if (comparator == CMP_LESS) comparator = CMP_GREATER;
	else if (comparator == CMP_LESS_EQ) comparator = CMP_GREATER_EQ;
	else if (comparator == CMP_GREATER) comparator = CMP_LESS;
	else if (comparator == CMP_GREATER_EQ) comparator = CMP_LESS_EQ;
}

void GroundedNumericCondition::reshapeTerms(GroundedVar* v)
{
    GroundedNumericExpression* left = &terms[0], *right = &terms[1];
    GroundedNumericExpressionType type = left->type;
    while (type == GE_SUM || type == GE_SUB || type == GE_MUL || type == GE_DIV) {
		bool cvOnLeft = left->terms[0].requiresNumericVariable(v->index);
        GroundedNumericExpression copyOfRight, copyOfLeft;
        copyOfRight.copyFrom(right);
        GroundedNumericExpressionType rightOp;
        if (cvOnLeft) {
            // cv + exp comp right -> cv comp right - exp
            // cv - exp comp right -> cv comp right + exp
            // cv * exp comp right -> cv comp right / exp
            // cv / exp comp right -> cv comp right * exp
            if (type == GE_SUM) rightOp = GE_SUB;
            else if (type == GE_SUB) rightOp = GE_SUM;
            else if (type == GE_MUL) rightOp = GE_DIV;
            else rightOp = GE_MUL;
            right->type = rightOp;
            right->terms.clear();
        ...
        } else if (type == GE_SUM || type == GE_MUL) {
            // exp + cv comp right -> cv comp right - exp
            // exp * cv comp right -> cv comp right / exp
            rightOp = type == GE_SUM ? GE_SUB : GE_DIV;
            right->type = rightOp;
            right->terms.clear();
            right->terms.emplace_back();
            right->terms.back().copyFrom(&copyOfRight);
...
bool GroundedNumericEffect::requiresNumericVariable(TVariable v)
{
	if (v == varIndex)
		return true;
	return exp.requiresNumericVariable(v);
}
...
string GroundedAction::toString(ParsedTask* task, vector<GroundedVar> &variables,
    ...
    for (unsigned int i = 0; i < controlVars.size(); i++)
        s += "\n\t" + controlVars[i].toString(task, variables, controlVars);
...
std::string GroundedControlVar::toString(ParsedTask* task, vector<GroundedVar>& variables, vector<GroundedControlVar>& controlVars)
{
    std::string res = name + " - " + (type == GCVT_INTEGER ? "integer" : "number");
    for (GroundedNumericCondition& c : conditions) {
        res += "\n    " + c.toString(task, variables, controlVars);
    }
	return res;
}

void GroundedControlVar::updateBound(GroundedNumericCondition* bound)
{
    double factor = precision < 0 ? 0.000001 : std::pow(10.0f, precision);
    float value = bound->terms[1].value;
    float limit = value;
    switch (bound->comparator) {
    case CMP_LESS:
        limit = std::floor(static_cast<double>(value) * factor - 1.0) / factor;
    case CMP_LESS_EQ:
        if (maxValue > limit)
            maxValue = limit;
        break;
