...
GroundedTask* Grounder::groundTask(PreprocessedTask *prepTask, bool keepStaticData) {
    ...
    for (unsigned int i = 0; i < numOps; i++) {
        ...
        bool positiveCond = false;
        for (OpFluent f : op->atStart.prec) {
            if (f.value.type != TERM_CONSTANT || f.value.index != gTask->task->CONSTANT_FALSE) {
                positiveCond = true;
                break;
            }
        }
        for (OpFluent f : op->overAllPrec) {
            if (f.value.type != TERM_CONSTANT || f.value.index != gTask->task->CONSTANT_FALSE) {
                positiveCond = true;
                break;
            }
    ...
    for (unsigned int i = 0; i < gTask->variables.size(); i++) {
        int fncIndex = gTask->variables[i].fncIndex;
        bool isInput = gTask->task->functions[fncIndex].isInput;
        gTask->variables[i].isInput = isInput; 
        if (isInput) {    
            gTask->variables[i].precision = gTask->task->functions[fncIndex].precision;
            for (Bounds b : gTask->task->bounds) {
                if (b.function == fncIndex) {
                    gTask->variables[i].bounds.emplace_back(b.cmp, b.value);
                }
            }
        }
    }
...
void Grounder::groundAction(GrounderOperator &op) {
    ...
    a.fullname = a.name;
    for (unsigned int i = 0; i < a.parameters.size(); i++)
        a.fullname += " " + gTask->task->objects[a.parameters[i]].name;
...
GroundedNumericExpression Grounder::groundNumericExpression(OpEffectExpression &exp, vector<unsigned int> &parameters) {
    ...
    case OEET_FLUENT:
         ...
         if (res.index == MAX_UNSIGNED_INT) {      // New variable
             int fnc_index = exp.fluent.fncIndex;
             if (gTask->task->functions[fnc_index].isInput)
...
void Grounder::checkControlActions()
{
	std::vector<GroundedVar*> controlVars;
    for (int i = 0; i < gTask->variables.size(); i++) {
		GroundedVar& v = gTask->variables[i];
		if (v.isInput) {
			controlVars.push_back(&v);
		}
    }
    if (controlVars.empty()) return;
    for (GroundedAction& a: gTask->actions) {
        for (GroundedVar* v : controlVars) {
            for (GroundedNumericCondition& c : a.startNumCond) {
                if (c.requiresNumericVariable(v->index)) {
                    addControlVar(v, a.controlVars);
                }
            }
            for (GroundedNumericCondition& c : a.overNumCond) {
                if (c.requiresNumericVariable(v->index)) {
                    addControlVar(v, a.controlVars);
                }
            }
            for (GroundedNumericCondition& c : a.endNumCond) {
                if (c.requiresNumericVariable(v->index)) {
                    addControlVar(v, a.controlVars);
                }
            }
            for (GroundedNumericEffect& c : a.startNumEff) {
                if (c.requiresNumericVariable(v->index))
                    addControlVar(v, a.controlVars);
            }
            for (GroundedNumericEffect& c : a.endNumEff) {
                if (c.requiresNumericVariable(v->index))
                    addControlVar(v, a.controlVars);
            }
        }
		if (!a.controlVars.empty()) {
			for (GroundedControlVar& cv : a.controlVars) {
                generateControlVarConditions(&cv, &a);
			}
            for (GroundedVar* v : controlVars) {
                int i = 0;
                while (i < a.startNumCond.size()) {
                    if (a.startNumCond[i].requiresNumericVariable(v->index)) {
                        a.startNumCond.erase(a.startNumCond.begin() + i);
					}
                    else {
                        i++;
                    }
                ...
                i = 0;
                while (i < a.overNumCond.size()) {
                    if (a.overNumCond[i].requiresNumericVariable(v->index)) {
                        a.overNumCond.erase(a.overNumCond.begin() + i);
                    }
                    else {
                        i++;
                    }
                ...
                i = 0;
                while (i < a.endNumCond.size()) {
                    if (a.endNumCond[i].requiresNumericVariable(v->index)) {
                        a.endNumCond.erase(a.endNumCond.begin() + i);
                    }
                    else {
                        i++;
                    }
...
bool Grounder::addControlVar(GroundedVar* v, std::vector<GroundedControlVar>& controlVars)
{
	for (GroundedControlVar& cv : controlVars) {
        if (cv.index == v->index) return false;
	}
	controlVars.emplace_back(v->index, gTask->task->functions[v->fncIndex].name);
	return true;
}

void Grounder::reshapeCondition(GroundedAction* a, GroundedNumericCondition* c, GroundedControlVar* cv)
{
	//cout << "Reshaping " << c->toString(gTask->task, gTask->variables, a->controlVars) << endl;
    GroundedVar* v = &gTask->variables[cv->index];
    GroundedNumericCondition copy;
    copy.copyFrom(c);
    copy.reshape(v);
	cv->conditions.push_back(copy);
	//cout << "Reshaped " << copy.toString(gTask->task, gTask->variables, a->controlVars) << endl;
}

void Grounder::generateControlVarConditions(GroundedControlVar* cv, GroundedAction* a)
{
    for (Function& f : gTask->task->functions) {
        if (f.isInput && gTask->task->functions[f.index].name.compare(cv->name) == 0) {
            cv->precision = f.precision;
            break;
        }
    }
    for (Bounds& b : gTask->task->bounds) {
		if (gTask->task->functions[b.function].name.compare(cv->name) == 0) {
            GroundedNumericCondition cond;
            cond.comparator = b.cmp;
			GroundedNumericExpression exp1;
			exp1.type = GE_VAR;
            exp1.index = cv->index;
			cond.terms.push_back(exp1);
            GroundedNumericExpression exp2;
			exp2.type = GE_NUMBER;
			exp2.value = b.value;
            cond.terms.push_back(exp2);
            cond.is_cvar_bound = true;
			cv->updateBound(&cond);
		}
    }
    for (GroundedNumericCondition& c : a->startNumCond) {
        if (c.requiresNumericVariable(cv->index)) {
			reshapeCondition(a, &c, cv);
        }
    }
    for (GroundedNumericCondition& c : a->overNumCond) {
        if (c.requiresNumericVariable(cv->index)) {
