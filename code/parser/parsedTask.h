...
enum TermType {
    TERM_CONSTANT = 0, TERM_PARAMETER = 1, TERM_CONTROL_VAR = 2
};

class Term {
public:
    TermType type;      // Constant, parameter or control variable
    unsigned int index; // Object index, parameter index or control variable index
    Term();
    Term(TermType type, unsigned int index);
...
enum NumericExpressionType {
    NET_NUMBER = 0, NET_FUNCTION = 1, NET_NEGATION = 2,
    NET_SUM = 3, NET_SUB = 4, NET_DIV = 5, NET_MUL = 6, NET_TERM = 7
};
...
public:
    ...
    inline bool equals(Literal& l) {
        if (fncIndex != l.fncIndex) return false;
        for (unsigned int i = 0; i < params.size(); i++)
            if (!params[i].equals(l.params[i])) return false;
        return true;
    }
};
...
enum TimeSpecifier { AT_START = 0, AT_END = 1, OVER_ALL = 2, NONE = 3 };

enum Comparator {
    CMP_EQ = 0, CMP_LESS = 1, CMP_LESS_EQ = 2, CMP_GREATER = 3,
    CMP_GREATER_EQ = 4, CMP_NEQ = 5, CMP_DUMMY = 6
};
...
enum GoalDescriptionType {
    GD_LITERAL = 0, GD_AND = 1, GD_NOT = 2, GD_OR = 3,
    GD_IMPLY = 4, GD_EXISTS = 5, GD_FORALL = 6, GD_F_CMP = 7, GD_EQUALITY = 8,
    GD_INEQUALITY = 9, GD_NEG_LITERAL = 10
};
...
public:
    ...
    std::string toString(const std::vector<Variable>& opParameters, const std::vector<Variable>& controlVars,
        const std::vector<Function>& functions, const std::vector<Object>& objects, const std::vector<Type>& taskTypes);
};

enum ConditionType { CT_AND = 0, CT_GOAL = 1, CT_FORALL = 2, CT_PREFERENCE = 3 };
...
enum EffectExpressionType {
    EE_NUMBER = 0, EE_DURATION = 1, EE_TERM = 2,
    EE_UNDEFINED = 3, EE_SHARP_T = 4, EE_OPERATION = 5, EE_FLUENT = 6
};

enum OperationType { OT_SUM = 0, OT_SUB = 1, OT_DIV = 2, OT_MUL = 3 };
...
enum Assignment {
    AS_ASSIGN = 0, AS_INCREASE = 1, AS_DECREASE = 2, AS_SCALE_UP = 3,
    AS_SCALE_DOWN = 4
};
...
enum TimedEffectType {
    TE_AND = 0, TE_NOT = 1, TE_LITERAL = 2, TE_ASSIGNMENT = 3,
    TE_NEG_LITERAL = 4, TE_OR = 5
};
...
enum DurativeEffectType {
    DET_AND = 0, DET_TIMED_EFFECT = 1, DET_FORALL = 2, DET_WHEN = 3,
    DET_ASSIGNMENT = 4
};
...
enum PreconditionType {
    PT_LITERAL = 0, PT_AND = 1, PT_NOT = 2, PT_OR = 3,
    PT_IMPLY = 4, PT_EXISTS = 5, PT_FORALL = 6, PT_F_CMP = 7, PT_EQUALITY = 8,
    PT_PREFERENCE = 9, PT_NEG_LITERAL = 10, PT_GOAL = 11
};
...
enum EffectType {
    ET_LITERAL = 0, ET_AND = 1, ET_NOT = 2, ET_FORALL = 3,
    ET_WHEN = 4, ET_ASSIGNMENT = 5, ET_NEG_LITERAL = 6
};
...
public:
    ...
    Comparator cmp;
    float value;
    Bounds(unsigned int fnc, Symbol s, float v);
    std::string toString(const std::vector<Function>& functions);
};

enum MetricType { MT_MINIMIZE = 0, MT_MAXIMIZE = 1, MT_NONE = 2 };

enum MetricExpressionType {
    MT_PLUS = 0, MT_MINUS = 1, MT_PROD = 2, MT_DIV = 3,
    MT_NUMBER = 4, MT_TOTAL_TIME = 5, MT_IS_VIOLATED = 6, MT_FLUENT = 7
};
...
enum ConstraintType {
    RT_AND = 0, RT_FORALL = 1, RT_PREFERENCE = 2, RT_AT_END = 3,
    RT_ALWAYS = 4, RT_SOMETIME = 5, RT_WITHIN = 6, RT_AT_MOST_ONCE = 7,
    RT_SOMETIME_AFTER = 8, RT_SOMETIME_BEFORE = 9, RT_ALWAYS_WITHIN = 10,
    RT_HOLD_DURING = 11, RT_HOLD_AFTER = 12, RT_GOAL_PREFERENCE = 13
};
