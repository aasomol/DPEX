...
ParsedTask* Parser::parseDomain(char* domainFileName) {
    ...
    while (syn->isSym(token, Symbol::OPEN_PAR)) {
        ...
        case Symbol::INPUT:
        case Symbol::FUNCTIONS:         parseFunctions(token->symbol == Symbol::INPUT); break;
...
ParsedTask* Parser::parseProblem(char* problemFileName) {
    ...
    while (syn->isSym(token, Symbol::OPEN_PAR)) {
        syn->readColon();
        token = syn->readSymbol(9, Symbol::REQUIREMENTS, Symbol::OBJECTS,
            Symbol::INIT, Symbol::GOAL, Symbol::CONSTRAINTS, Symbol::METRIC,
            Symbol::LENGTH, Symbol::BOUNDS, Symbol::PRECISION);
        switch (token->symbol) {
        case Symbol::REQUIREMENTS:      parseRequirements();    break;
        ...
        case Symbol::BOUNDS:            parseBounds();          break;
        case Symbol::PRECISION:         parsePrecision();       break;
        default:;
        }
        token = syn->readSymbol(2, Symbol::OPEN_PAR, Symbol::CLOSE_PAR);
    }
...
void Parser::parseFunctions(bool isInput) {
...
EffectExpression Parser::parseEffectExpression(const vector<Variable>& parameters, const vector<Variable>& controlVars, bool isAssign) {
    ...
    case Symbol::NAME:
        if (token->description.compare("?duration") == 0) exp.type = EE_DURATION;
            ...
            unsigned int functionIndex = task->getFunctionIndex(token->description);
            if (functionIndex == MAX_UNSIGNED_INT) {
                exp.type = EE_TERM;
                syn->undoToken();
                exp.term = parseTerm(parameters, controlVars);
            }
            else {
                if (!task->isNumericFunction(functionIndex))
                    syn->notifyError("Numeric Function '" + token->description + "' undefined");
                exp.type = EE_FLUENT;
                syn->undoToken();
...
void Parser::parseBounds()
{
    Token* token = syn->readSymbol(2, Symbol::OPEN_PAR, Symbol::CLOSE_PAR);
    while (syn->isSym(token, Symbol::OPEN_PAR)) {
        token = syn->readSymbol(5, Symbol::LESS_EQ, Symbol::LESS, Symbol::GREATER,
            Symbol::GREATER_EQ, Symbol::EQUAL);
        Symbol cmp = token->symbol;
        syn->openPar();
        string name = syn->readName();
        unsigned int fnc_index = task->getFunctionIndex(name);
        if (fnc_index == MAX_UNSIGNED_INT || !task->isNumericFunction(fnc_index) || !task->functions[fnc_index].isInput)
            syn->notifyError("Name '" + name + "' is not an input");
        syn->closePar();
        token = syn->readSymbol(Symbol::NUMBER);
        task->bounds.emplace_back(fnc_index, cmp, token->value);
        syn->closePar();
        token = syn->readSymbol(2, Symbol::OPEN_PAR, Symbol::CLOSE_PAR);
    }
}

void Parser::parsePrecision()
{
    Token* token = syn->readSymbol(2, Symbol::OPEN_PAR, Symbol::CLOSE_PAR);
    while (syn->isSym(token, Symbol::OPEN_PAR)) {
        syn->readSymbol(Symbol::EQUAL);
        syn->openPar();
        string name = syn->readName();
        unsigned int fnc_index = task->getFunctionIndex(name);
        if (fnc_index == MAX_UNSIGNED_INT || !task->isNumericFunction(fnc_index) || !task->functions[fnc_index].isInput)
        ...
        syn->closePar();
        token = syn->readSymbol(Symbol::NUMBER);
        task->functions[fnc_index].precision = (int)token->value;
        syn->closePar();
        token = syn->readSymbol(2, Symbol::OPEN_PAR, Symbol::CLOSE_PAR);
    }
}
