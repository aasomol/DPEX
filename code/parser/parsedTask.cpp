...
string Function::toString(const vector<Type>& taskTypes) {
    ...
    res += ") - ";
    if (valueTypes.size() == 1) res += taskTypes[valueTypes[0]].name;
    else {
        res += "(either";
        for (unsigned int i = 0; i < valueTypes.size(); i++)
            res += " " + taskTypes[valueTypes[i]].name;
        res += ")";
    }
    return res;
}
...
Bounds::Bounds(unsigned int fnc, Symbol s, float v)
{
    this->function = fnc;
    this->value = v;
    if (s == Symbol::EQUAL) this->cmp = Comparator::CMP_EQ;
    else if (s == Symbol::LESS_EQ) this->cmp = Comparator::CMP_LESS_EQ;
    else if (s == Symbol::LESS) this->cmp = Comparator::CMP_LESS;
    else if (s == Symbol::GREATER) this->cmp = Comparator::CMP_GREATER;
    else this->cmp = Comparator::CMP_GREATER_EQ;
}

std::string Bounds::toString(const std::vector<Function>& functions)
{
    string s = "(";
    switch (this->cmp) {
    case CMP_EQ:         s += "=";      break;
    ...
    s += " (" + functions[this->function].name + ") " + to_string(this->value);
    return s + ")";
}
